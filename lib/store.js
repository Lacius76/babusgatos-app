...
function createRedisStore(redis) {
  ...
  store.transaction = async (fn) => fn(store);
  return store;
}
...
function createBlobStore() {
  ...
  function writeValue(data, key, value, options = {}) {
    if (options.ex) {
      data.values[key] = { value, expiresAt: Date.now() + options.ex * 1000 };
    } else {
  ...
  function makeTx(data) {
    return {
      ...
      async set(key, value, options = {}) {
        writeValue(data, key, value, options);
      },
      async del(key) {
        delete data.values[key];
      },
