...
async function saveSubscription(subscription, towns) {
  ...
  const existing = await store.get(subscriptionKey(sub.endpoint));
  const oldTowns = existing?.towns || [];

  for (const oldTown of oldTowns) {
    if (!townList.includes(oldTown)) {
      await store.srem(townKey(oldTown), sub.endpoint);
    }
  }

  for (const town of townList) {
    await store.sadd(townKey(town), sub.endpoint);
  }

  await store.set(subscriptionKey(sub.endpoint), {
    subscription: sub,
    towns: townList,
    updatedAt: new Date().toISOString(),
  });

  console.log("Push feliratkozás mentve:", { backend: store.backend, towns: townList });
  return { towns: townList, backend: store.backend };
}
...
async function removeSubscription(endpoint) {
  ...
  for (const town of towns) {
    await store.srem(townKey(town), endpoint);
  }

  await store.del(subscriptionKey(endpoint));
  return { removed: true };
}
...
async function sendTestPush(endpoint) {
  ...
  const payload = {
    title: PUSH_TITLE,
    body: "🔔 Teszt értesítés – ha ezt látod, működik!",
    url: "/",
  };

  const result = await sendPushToEndpoint(endpoint, payload);
  console.log("Teszt push eredménye:", { endpoint: endpoint.slice(0, 48), ...result });
