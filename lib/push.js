...
const NAGYCENK_CENTER = { lat: 47.5833, lon: 16.7167 };
...
async function maybeNotifyNagycenkArrival(lat, lon) {
  if (!Number.isFinite(lat) || !Number.isFinite(lon)) return { skipped: true, reason: "no_coords" };
  if (!isWithinNagycenk(lat, lon)) return { skipped: true, reason: "outside_radius" };
  if (!configureWebPush()) return { skipped: true, reason: "vapid_missing" };

  const redis = await getRedis();
  if (!redis) return { skipped: true, reason: "kv_unavailable" };
  ...
  await markNotified(NOTIFY_TOWN);
  return { notified: true, ...sent };
}
